...
class adminBack{
    ...
    function product_by_ctg($id){
        $query = "SELECT * FROM products_info_ctg WHERE ctg_id=$id";
        if(mysqli_query($this->conn, $query)){
    ...
    function related_product($id){
        $query= "SELECT * FROM products_info_ctg WHERE ctg_id=$id ORDER BY pdt_id DESC LIMIT 2";
        if(mysqli_query($this->conn, $query)){
            $proinfo = mysqli_query($this->conn, $query);
            return $proinfo;
        }
    ...
    function user_register($data){
        ...
        if($row==1){
            ...
            if(strlen($user_mobile)< 11 or strlen($user_mobile)>11){
                $msg = "Your Mobile Number Not Be Less Than or Greater Then 11 Digit";
                return $msg;
            }else{
        ...
        if(mysqli_query($this->conn, $query)){
            $msg = "Your account succesfully registered!";
            return $msg;
        }
            }
        }
    ...
    function user_login($data){
        ...
        if(mysqli_query($this->conn,$query)){
            ...
            if($user_info){
                ...
                ['user_email'];
                $_SESSION['user_pass'] = $user_info
                ['user_password'];
                $_SESSION['user_name'] = $user_info
                ['user_name'];
            }else{
